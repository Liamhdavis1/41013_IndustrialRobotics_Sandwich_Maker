...
# ---------------- Collision detection helpers ----------------
    def get_algebraic_dist(self,points, center_point, radii):
        return np.sum(((points - center_point)/radii)**2, axis=1)

    def detect_collisions(self, ellipsoid_robot, points):
        collisions = []
        for i in range(len(ellipsoid_robot.ellipsoid_matrices)):
            ...
            algebra_dist = self.get_algebraic_dist(local_points, [0, 0, 0], radii)
            inside_indices = np.where(algebra_dist < 1)[0]
            if inside_indices.size > 0:
                collisions.append((i, inside_indices.size))
        return collisions

    def load_mesh_points(self, stl_path, num_points=10000):
    ...
    #     # ---------------- Collision setup ----------------
    #     bench_points = self.load_mesh_points(ENV["bench"][os.path.join(current_path, "env", "bread_rack.stl")], num_points=8000)
    ...
    #         # Check collisions
    #         collisions_XArm = self.detect_collisions(ellipsoid_XArm, bench_points)
    #         collisions_UR3 = self.detect_collisions(ellipsoid_UR3, bench_points)
    #         collisions_irb = self.detect_collisions(ellipsoid_irb, bench_points)
    ...
    def Collsion(self, robot):
        stl_path = os.path.join(os.path.dirname(__file__), "env", "benchv2.stl")
        bench_points = self.load_mesh_points(stl_path, num_points=8000)
        ellipsoid_robot = EllipsoidRobot(robot)
        ellipsoid_robot.ellipsoid_for_robot_links(robot.q)
        collisions_robot = self.detect_collisions(ellipsoid_robot, bench_points)
        if collisions_robot:
                print(f"Collision detected for XArm6: {len(collisions_robot)} links involved")
